...
import { readableStreamToString, BrexBudgetsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrexBudgetsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrexBudgetsError(e, e.response?.data)
                }
                if (innerError instanceof BrexBudgetsError) {
                    // Got "BrexBudgetsError" from the above try block
                    throw innerError;
                }
